    ...
    describe("error handling", () => {
      it("should throw error with UNKNOWN_PROVIDER code when provider does not exist", () => {
        try {
          ...
          expect((error as NotFoundError).details.providerId).toBe(
      ...
      it("should throw error with MISSING_API_KEY code when no API key is configured", () => {
        try {
          ...
          expect((error as InternalError).code).toBe(
            "PROVIDER:MISSING_API_KEY",
          );
          expect((error as InternalError).status).toBe(500);
          expect((error as InternalError).details.envNames).toContain(
            "ZHIPU_API_KEY",
          );
        }
      });
  ...
  describe("listModels", () => {
    describe("error handling", () => {
      ...
      it("should throw error with MISSING_API_KEY code when no API key is configured", async () => {
        try {
          console.log("process.env.ZHIPU_API_KEY", process.env.ZHIPU_API_KEY);
          const a = await listModels("zhipuai-coding-plan");
          console.log(a);
    ...
    describe("API error handling", () => {
      ...
      it("should throw PROVIDER_API_ERROR when network error occurs", async () => {
        ...
        try {
          ...
          expect((error as InternalError).code).toBe("PROVIDER:API_ERROR");
          expect((error as InternalError).details.message).toContain(
            "Network error",
          );
        }
      });
